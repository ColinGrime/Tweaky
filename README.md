...
* `durability-indicator` - Adds an indicator when your important items are about to break.
...
* `horse-randomization` - Right-click a horse with a brush to randomize its color and style.
...
* `vehicle-mobs` - Right-click a mob with a vehicle to place the vehicle down with them in it.
...
* `villager-protection` - Prevents you from hitting villagers by accident.
...
* `/drops` - Allows players to toggle the 'drops-filter' tweak. Requires it to be enabled. 
* `/nightvision` - Allows players to toggle the 'night-vision' tweak. Requires it to be enabled.
