...
public final class Experience {
	 ...
	 */
	public static int fromPlayer(@Nonnull Player player) {
		return fromLevel(player.getLevel()) + Math.round(getExpToNext(player.getLevel()) * player.getExp());
	}
	 ...
	 */
	public static double toLevel(long exp) {
		int level = toWholeLevel(exp);

		// Get remaining exp progressing towards next level. Cast to float for next bit of math.
		float remainder = exp - (float) fromLevel(level);
	 ...
	 * @return the level calculated
	 */
	public static int toWholeLevel(long exp) {
		if (exp > 1395) {
	 ...
	 */
	private static void changeExp(@Nonnull Player player, int exp) {
		exp += fromPlayer(player);

		if (exp < 0) {
			exp = 0;
		}

		double levelAndExp = toLevel(exp);
