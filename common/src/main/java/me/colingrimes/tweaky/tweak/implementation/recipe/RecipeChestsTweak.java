...
public class RecipeChestsTweak extends RecipeTweak {

	private static final List<Material> LOGS = List.copyOf(Tag.LOGS.getValues());

	public RecipeChestsTweak(@Nonnull Tweaky plugin) {
		super(plugin, "recipe_chests");
	}

	@Nonnull
	@Override
	protected Recipe recipe(@Nonnull NamespacedKey key) {
		return new ShapedRecipe(key, new ItemStack(Material.CHEST, 4))
				.shape("LLL",
					   "L L",
					   "LLL")
